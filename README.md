...
記事シリーズ **HTML basics**（[001 から読む](https://www.visionnurture.com/html_basics_for_beginner_001/)）と一緒に使うコードです。
...
| `tools/` | 測るためのコード。記事の数値はここで測ったものです |
...
- 記事に載っていないコードはありません。記事を読むだけで追試できます
...
> `measure-bundle.mjs` と `measure-react-interop.mjs` は `compare/008-*` 配下の**別パッケージ**をビルドします。依存の導入は各コマンドが自分で行うため、上記をそのまま実行できます（初回のみ時間がかかります）。
